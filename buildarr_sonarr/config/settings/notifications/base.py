...
class NotificationTriggers(SonarrConfigBase):
    ...
    """
    Notify when movies are added to Sonarr's library to monitor.
    """
    ...
    """
    Notify when Sonarr gets updated to a new version.
    """
...
class Notification(SonarrConfigBase):
    # Notification connection configuration base class.
    ...
    """
    Sonarr tags to associate this notification connection with.
    """
    ...
    def _get_api_schema(self, schemas: List[sonarr.NotificationResource]) -> Dict[str, Any]:
        return {
            k: v
            for k, v in next(
    ...
    def _create_remote(
        ...
        api_notification_schemas: List[sonarr.NotificationResource],
        tag_ids: Mapping[str, int],
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.NotificationApi(api_client).create_notification(
                notification_resource=sonarr.NotificationResource.from_dict(remote_attrs),
            )
    ...
    def _update_remote(
        ...
        api_notification_schemas: List[sonarr.NotificationResource],
        tag_ids: Mapping[str, int],
        api_notification: sonarr.NotificationResource,
    ) -> bool:
        (
            triggers_updated,
        ...
        if triggers_updated or base_updated:
            ...
            with sonarr_api_client(secrets=secrets) as api_client:
                sonarr.NotificationApi(api_client).update_notification(
    ...
    def _delete_remote(self, secrets: SonarrSecrets, notification_id: int) -> None:
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.NotificationApi(api_client).delete_notification(id=notification_id)
