...
class SonarrMetadataSettings(SonarrConfigBase):
    ...
    """
    The country to use for movie certifications in Sonarr.
    ...
    def from_remote(cls, secrets: SonarrSecrets) -> Self:
        with sonarr_api_client(secrets=secrets) as api_client:
            api_config = sonarr.MetadataConfigApi(api_client).get_metadata_config()
            metadata_api = sonarr.MetadataApi(api_client)
            api_metadata_schemas: Dict[str, sonarr.MetadataResource] = {
                api_schema.implementation: api_schema
                for api_schema in metadata_api.list_metadata_schema()
            }
            api_metadatas: Dict[str, sonarr.MetadataResource] = {
        ...
        if EmbyLegacyMetadata._implementation not in api_metadatas:
            raise RuntimeError(
                "Unable to find Emby (Legacy) metadata on Sonarr, database might be corrupt",
            )
    ...
    def update_remote(
        ...
        if config_changed:
            with sonarr_api_client(secrets=secrets) as api_client:
                sonarr.MetadataConfigApi(api_client).update_metadata_config(
                    id=str(api_config.id),
                    metadata_config_resource=sonarr.MetadataConfigResource.from_dict(
