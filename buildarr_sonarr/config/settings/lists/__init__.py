...
from .sonarr_import import SonarrImportList
...
class SonarrListsSettings(SonarrConfigBase):
    """
    ...
    def from_remote(cls, secrets: SonarrSecrets) -> Self:
        with sonarr_api_client(secrets=secrets) as api_client:
            api_importlist_config = sonarr.ImportListConfigApi(api_client).get_import_list_config()
            api_importlists = sonarr.ImportListApi(api_client).list_import_list()
            quality_profile_ids: Dict[str, int] = (
                {
                    profile.name: profile.id
                    for profile in sonarr.QualityProfileApi().list_quality_profile()
                }
    ...
    def _update_remote_config(
        ...
        if updated:
            with sonarr_api_client(secrets=secrets) as api_client:
                importlist_config_api = sonarr.ImportListConfigApi(api_client)
                ...
                importlist_config_api.update_import_list_config(
                    ...
                    import_list_config_resource=sonarr.ImportListConfigResource.from_dict(
    ...
    def _update_remote_definitions(
        ...
        changed = False
        with sonarr_api_client(secrets=secrets) as api_client:
            importlist_api = sonarr.ImportListApi(api_client)
            api_importlist_schemas = importlist_api.list_import_list_schema()
            api_importlists: Dict[str, sonarr.ImportListResource] = {
                api_importlist.name: api_importlist
                for api_importlist in importlist_api.list_import_list()
            }
            quality_profile_ids: Dict[str, int] = {
                profile.name: profile.id
                for profile in sonarr.QualityProfileApi(api_client).list_quality_profile()
            }
            tag_ids: Dict[str, int] = (
                {tag.label: tag.id for tag in sonarr.TagApi(api_client).list_tag()}
    ...
    def delete_remote(self, tree: str, secrets: SonarrSecrets, remote: Self) -> bool:
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            importlist_ids: Dict[str, int] = {
                ...
                for api_importlist in sonarr.ImportListApi(api_client).list_import_list()
            }
