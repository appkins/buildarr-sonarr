...
class ImportList(SonarrConfigBase):
    ...
    refer to this guide on [WikiArr](https://wiki.servarr.com/sonarr/settings#import-lists).
    ...
    * `none`
    ...
    def _get_api_schema(self, schemas: Iterable[sonarr.ImportListResource]) -> Dict[str, Any]:
        return {
            k: v
            for k, v in next(
    ...
    def _create_remote(
        ...
        api_importlist_schemas: Iterable[sonarr.ImportListResource],
        quality_profile_ids: Mapping[str, int],
        tag_ids: Mapping[str, int],
        importlist_name: str,
    ) -> None:
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.ImportListApi(api_client).create_import_list(
                import_list_resource=sonarr.ImportListResource.from_dict(remote_attrs),
            )
    ...
    def _update_remote(
        ...
        quality_profile_ids: Mapping[str, int],
        tag_ids: Mapping[str, int],
        api_importlist: sonarr.ImportListResource,
    ) -> bool:
        ...
        if updated:
            ...
            with sonarr_api_client(secrets=secrets) as api_client:
                sonarr.ImportListApi(api_client).update_import_list(
    ...
    def _delete_remote(self, secrets: SonarrSecrets, importlist_id: int) -> None:
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.ImportListApi(api_client).delete_import_list(id=importlist_id)
