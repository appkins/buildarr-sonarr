...
class SonarrIndexersSettings(SonarrConfigBase):
    """

    !!! note

        Instead of manually configuring indexers for each of your Sonarr instances,
        ...
        This is particularly convenient when managing more than one Sonarr instance.

        When managing indexers using Prowlarr, do not add any indexer definitions
        to this Sonarr instance in Buildarr, and ensure `delete_unmanaged` is set to `false`.
    ...
    def from_remote(cls, secrets: SonarrSecrets) -> Self:
        with sonarr_api_client(secrets=secrets) as api_client:
            api_indexer_config = sonarr.IndexerConfigApi(api_client).get_indexer_config()
            indexer_api = sonarr.IndexerApi(api_client)
            api_indexer_schemas: Dict[str, sonarr.IndexerResource] = {
                api_schema.implementation: api_schema
                for api_schema in indexer_api.list_indexer_schema()
            }
            api_indexers: Dict[str, sonarr.IndexerResource] = {
                api_indexer.name: api_indexer for api_indexer in indexer_api.list_indexer()
            }
            downloadclient_ids: Dict[str, int] = (
                {dc.name: dc.id for dc in sonarr.DownloadClientApi().list_download_client()}
                if any(api_indexer.download_client_id for api_indexer in api_indexers.values())
                else {}
            )
            tag_ids: Dict[str, int] = (
                {tag.label: tag.id for tag in sonarr.TagApi(api_client).list_tag()}
    ...
    def update_remote(
        ...
        if config_updated:
            with sonarr_api_client(secrets=secrets) as api_client:
                sonarr.IndexerConfigApi(api_client).update_indexer_config(
    ...
    def delete_remote(self, tree: str, secrets: SonarrSecrets, remote: Self) -> bool:
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            indexer_ids: Dict[str, int] = {
                ...
                for api_indexer in sonarr.IndexerApi(api_client).list_indexer()
            }
