...
class TorrentIndexer(Indexer):
    ...
    All values supported by your Sonarr instance version can be defined.
    As of Sonarr v4.7.5, the following flags are available:
    ...
    * `ahd-userrelease`
    ...
    @field_validator("required_flags")
    ...
    @classmethod
    def _flag_decode(cls, api_schema: sonarr.IndexerResource, value: int) -> str:
        field: sonarr.Field = next(f for f in api_schema.fields if f.name == "requiredFlags")
    ...
    @classmethod
    def _flag_encode(cls, api_schema: sonarr.IndexerResource, value: str) -> int:
        field: sonarr.Field = next(f for f in api_schema.fields if f.name == "requiredFlags")
        select_options: List[sonarr.SelectOption] = field.select_options
        for option in select_options:
            option_name: str = option.name
