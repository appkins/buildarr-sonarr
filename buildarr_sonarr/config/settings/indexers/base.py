...
class Indexer(SonarrConfigBase):
    ...
    def _get_base_remote_map(
        cls,
        api_schema: sonarr.IndexerResource,
    ...
    @classmethod
    def _language_decode(cls, api_schema: sonarr.IndexerResource, value: str) -> str:
        field: sonarr.Field = next(f for f in api_schema.fields if f.name == "multiLanguages")
        select_options: List[sonarr.SelectOption] = field.select_options
    ...
    @classmethod
    def _language_encode(cls, api_schema: sonarr.IndexerResource, value: str) -> int:
        field: sonarr.Field = next(f for f in api_schema.fields if f.name == "multiLanguages")
        select_options: List[sonarr.SelectOption] = field.select_options
        for option in select_options:
            option_name: str = option.name
    ...
    def _from_remote(
        ...
        downloadclient_ids: Mapping[str, int],
        tag_ids: Mapping[str, int],
        api_indexer: sonarr.IndexerResource,
    ) -> Self:
        return cls(
            **cls.get_local_attrs(
                remote_map=(
    ...
    def _create_remote(
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.IndexerApi(api_client).create_indexer(
                indexer_resource=sonarr.IndexerResource.from_dict(remote_attrs),
            )
    ...
    def _update_remote(
        ...
        if updated:
            ...
            with sonarr_api_client(secrets=secrets) as api_client:
                sonarr.IndexerApi(api_client).update_indexer(
    ...
    def _delete_remote(self, secrets: SonarrSecrets, indexer_id: int) -> None:
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.IndexerApi(api_client).delete_indexer(id=indexer_id)
