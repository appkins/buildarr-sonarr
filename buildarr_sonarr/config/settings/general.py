...
class GeneralSettings(SonarrConfigBase):
    ...
    @classmethod
    def _from_remote(cls, secrets: SonarrSecrets, remote_attrs: Mapping[str, Any]) -> Self:
        return cls(
            **cls.get_local_attrs(
                remote_map=(
                    cls._remote_map
...
class HostGeneralSettings(GeneralSettings):
    ...
    **Changing any of these settings require a restart of Sonarr to take effect.**
    ...
    Instead of enabling HTTPS directly on Sonarr, it is recommended
...
class SecurityGeneralSettings(GeneralSettings):
    ...
    * `none` - No authentication (Sonarr V4 only)
    * `basic` - Authentication using HTTP basic auth (browser popup)
    * `form`/`forms` - Authentication using a login page
    * `external` - External authentication using a reverse proxy (Sonarr V5 and above)
    ...
    *Changed in version 0.2.0: Added support for the `external` authentication method.*
    ...
    Available on Sonarr V5 and above. Unused when managing Sonarr V4 instances.
...
class SonarrGeneralSettings(SonarrConfigBase):
    """
    ...
    def from_remote(cls, secrets: SonarrSecrets) -> Self:
        with sonarr_api_client(secrets=secrets) as api_client:
            remote_attrs = sonarr.HostConfigApi(api_client).get_host_config().to_dict()
        return cls(
    ...
    def update_remote(
        ...
        if any(
            ...
            with sonarr_api_client(secrets=secrets) as api_client:
                host_config_api = sonarr.HostConfigApi(api_client)
                remote_attrs = {
                ...
                host_config_api.update_host_config(
                    ...
                    host_config_resource=sonarr.HostConfigResource.from_dict(remote_attrs),
                )
            return True
        return False
