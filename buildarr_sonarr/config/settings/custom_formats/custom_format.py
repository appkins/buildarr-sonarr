...
from ....exceptions import SonarrConfigUnsupportedError
...
class CustomFormat(SonarrConfigBase):
    # Custom format definition configuration.
    ...
    * If this is a manually defined custom format, the default score will be 0.
    ...
    # Sonarr/Sonarr Custom Format JSON specification:
    # https://github.com/TRaSH-Guides/Guides/blob/master/CONTRIBUTING.md#sonarrsonarr-custom-format-json
    ...
    def _post_init_render(
        ...
        for customformat_file in (
            state.trash_metadata_dir / "docs" / "json" / "sonarr" / "cf"
        ).iterdir():
            with customformat_file.open() as f:
                ...
                if cast(str, trash_customformat["trash_id"]).lower() == self.trash_id:
                    if "default_score" not in self.model_fields_set:
                        try:
                    ...
                    for trash_condition in trash_customformat["specifications"]:
                        ...
                        if condition_name not in self.conditions:
                            ...
                            try:
                                ...
                                raise SonarrConfigUnsupportedError(
                                    (
                            ...
                            self.conditions[
                                ...
                                api_condition=sonarr.CustomFormatSpecificationSchema.from_dict(
                                    api_condition_dict,
                                ),
                            )
        ...
        raise ConfigTrashIDNotFoundError(
            f"Unable to find Sonarr custom format profile with trash ID '{self.trash_id}'",
        )
    ...
    def _from_remote(
        ...
        api_condition_schema_dicts: Dict[str, Dict[str, Any]],
        api_customformat: sonarr.CustomFormatResource,
    ) -> CustomFormat:
        conditions: Dict[str, ConditionType] = {}
        for api_condition in cast(
            List[sonarr.CustomFormatSpecificationSchema],
    ...
    def _create_remote(
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.CustomFormatApi(api_client).create_custom_format(
                custom_format_resource=sonarr.CustomFormatResource.from_dict(
                    {
    ...
    def _update_remote(
        ...
        api_conditions: Dict[str, sonarr.CustomFormatSpecificationSchema] = {
        ...
        if changed:
            with sonarr_api_client(secrets=secrets) as api_client:
                sonarr.CustomFormatApi(api_client).update_custom_format(
    ...
    def _delete_remote(self, secrets: SonarrSecrets, customformat_id: int) -> None:
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.CustomFormatApi(api_client).delete_custom_format(id=customformat_id)
