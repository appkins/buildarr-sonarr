...
class SonarrCustomFormatsSettings(SonarrConfigBase):
    # Custom format settings configuration.
    ...
    def _post_init_render(self, secrets: SonarrSecrets) -> None:
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.CustomFormatApi(api_client)
            # TODO: Replace with CustomFormatApi.get_custom_format_schama when fixed.
            # https://github.com/devopsarr/sonarr-py/issues/36
    ...
    def from_remote(cls, secrets: SonarrSecrets) -> Self:
        with sonarr_api_client(secrets=secrets) as api_client:
            customformat_api = sonarr.CustomFormatApi(api_client)
            # TODO: Replace with CustomFormatApi.get_custom_format_schama when fixed.
            # https://github.com/devopsarr/sonarr-py/issues/36
            api_condition_schema_dicts: Dict[str, Dict[str, Any]] = {
                api_schema_dict["implementation"]: api_schema_dict
