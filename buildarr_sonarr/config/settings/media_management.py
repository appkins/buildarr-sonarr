...
class RootFoldersSettings(SonarrConfigBase):
    """
    Root folders are paths used by Sonarr as either a target folder
    for newly downloaded releases, or a source folder for Sonarr to scan existing media.

    Root folders are assigned to releases when they are submitted for monitoring in Sonarr.
    ...
    """
    Delete root folders from the remote Sonarr instance if
    ...
    """
    Define root folder paths to add to Sonarr here.
    """
    ...
    def update_remote(
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            rootfolder_api = sonarr.RootFolderApi(api_client)
            ...
            for i, root_folder in enumerate(self.definitions):
                if root_folder in rootfolder_ids:
                    ...
                    rootfolder_api.create_root_folder(
                        root_folder_resource=sonarr.RootFolderResource(path=root_folder),
                    )
                    changed = True
        return changed
...
class SonarrMediaManagementSettings(SonarrConfigBase):
    ...
    * `never`
    ...
    !!! warning
        ...
        Sonarr will not automatically detect changes to files
    ...
    * `drwxrwxrwx`/`777`
    ...
    This only works if the user running Sonarr is the owner of the file.
    Ideally, the download client should be using  the same group as Sonarr.
    """
    ...
    def from_remote(cls, secrets: SonarrSecrets) -> Self:
        with sonarr_api_client(secrets=secrets) as api_client:
            api_naming_config = sonarr.NamingConfigApi(api_client).get_naming_config()
            api_mediamanagement_config = sonarr.MediaManagementConfigApi(
                api_client,
    ...
    def _update_remote_naming(
        ...
        if updated:
            with sonarr_api_client(secrets=secrets) as api_client:
                naming_api = sonarr.NamingConfigApi(api_client)
                api_naming_config = naming_api.get_naming_config()
                naming_api.update_naming_config(
                    id=str(api_naming_config.id),
                    naming_config_resource=sonarr.NamingConfigResource.from_dict(
    ...
    def _update_remote_mediamanagement(
        ...
        if updated:
            with sonarr_api_client(secrets=secrets) as api_client:
                mediamanagement_api = sonarr.MediaManagementConfigApi(api_client)
                ...
                mediamanagement_api.update_media_management_config(
                    ...
                    media_management_config_resource=(
                        sonarr.MediaManagementConfigResource.from_dict(
