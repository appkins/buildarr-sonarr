...
class QualityGroup(SonarrConfigBase):
    ...
    def encode(self, group_id: int, api_qualities: Mapping[str, sonarr.Quality]) -> Dict[str, Any]:
        return {
            "id": group_id,
            "name": self.name,
            "allowed": True,
...
class QualityProfile(SonarrConfigBase):
    ...
    Once this quality is reached, Sonarr will no longer upgrade movie releases
    ...
    Once this number is reached, Sonarr will no longer upgrade movie releases
    ...
    All languages supported by your Sonarr instance version can be defined here.
    ...
    * `portuguese-brazil`
    ...
    def _get_remote_map(
        cls,
        api_qualities: Mapping[str, sonarr.Quality] = {},
        api_customformats: Mapping[str, sonarr.CustomFormatResource] = {},
        api_languages: Mapping[str, sonarr.LanguageResource] = {},
    ...
    def _create_remote(
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.QualityProfileApi(api_client).create_quality_profile(
                quality_profile_resource=sonarr.QualityProfileResource.from_dict(
                    {
                        "name": profile_name,
    ...
    def _update_remote(
        ...
        api_customformats: Mapping[str, sonarr.CustomFormatResource],
        api_languages: Mapping[str, sonarr.LanguageResource],
        api_profile: sonarr.QualityProfileResource,
    ) -> bool:
        group_ids: Dict[str, int] = {
            quality_group.name: (1000 + i)
            for i, quality_group in enumerate(
        ...
        if updated:
            with sonarr_api_client(secrets=secrets) as api_client:
                sonarr.QualityProfileApi(api_client).update_quality_profile(
    ...
    def _delete_remote(self, secrets: SonarrSecrets, profile_id: int) -> None:
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.QualityProfileApi(api_client).delete_quality_profile(id=profile_id)


class SonarrQualityProfilesSettings(SonarrConfigBase):
    """
    Quality profiles are used by Sonarr as the guideline for
    desired quality when searching for releases.

    When media is requested for download in Sonarr, the user selects a quality profile
    ...
    where the name of the definition becomes the name of the quality profile in Sonarr.
    """
    ...
    **If there are no quality profiles defined in Buildarr and
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            return cls(
                definitions={
                    ...
                    for api_profile in sonarr.QualityProfileApi(api_client).list_quality_profile()
                },
            )
    ...
    def update_remote(
        ...
        updated = False
        with sonarr_api_client(secrets=secrets) as api_client:
            api_profiles: Dict[str, sonarr.QualityProfileResource] = {
                api_profile.name: api_profile
                for api_profile in sonarr.QualityProfileApi(api_client).list_quality_profile()
            }
            api_qualities: Dict[str, sonarr.Quality] = {
                api_qualitydefinition.title: api_qualitydefinition.quality
                for api_qualitydefinition in sorted(
                    sonarr.QualityDefinitionApi(api_client).list_quality_definition(),
            ...
            api_languages: Dict[str, sonarr.LanguageResource] = {
                language_parse(api_language.name): api_language
                for api_language in sonarr.LanguageApi(api_client).list_language()
            }
