    ...
    * `torrent-only` (Only Torrent)
    ...
    def _create_remote(
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.DelayProfileApi(api_client).create_delay_profile(
                delay_profile_resource=sonarr.DelayProfileResource.from_dict(
                    {
                        "order": order,
    ...
    def _update_remote(
        ...
        remote: DelayProfile,
        tag_ids: Mapping[str, int],
        api_profile: sonarr.DelayProfileResource,
        order: int,
    ) -> bool:
        ...
        if updated:
            with sonarr_api_client(secrets=secrets) as api_client:
                sonarr.DelayProfileApi(api_client).update_delay_profile(
    ...
    def _delete_remote(
        ...
        with sonarr_api_client(secrets=secrets) as api_client:
            sonarr.DelayProfileApi(api_client).delete_delay_profile(profile_id)
...
class SonarrDelayProfilesSettings(SonarrConfigBase):
    ...
    allowing Sonarr to grab releases that better match your preferences.
    ...
    [WikiArr](https://wiki.servarr.com/sonarr/settings#delay-profiles).
    """
    ...
    def from_remote(cls, secrets: SonarrSecrets) -> Self:
        with sonarr_api_client(secrets=secrets) as api_client:
            api_profiles: List[sonarr.DelayProfileResource] = sorted(
                sonarr.DelayProfileApi(api_client).list_delay_profile(),
